...
public:
	...
	//MOCK_METHOD(void, callback, (const int &sock_fd, const std::string &data), (override));
    MOCK_METHOD(void, ThreadListen, (), (const));
    MOCK_METHOD(void, BindPort, (const int &port_num), (const));
    void ForCallbackTest(const int &sock_fd, const std::string &data)
    {
...
TEST(HttpListener_MOCK, Listening)
{
    ...
    EXPECT_CALL(http_listener->mock_callback, Call(::testing::_, ::testing::_)).Times(::testing::AtLeast(1));
